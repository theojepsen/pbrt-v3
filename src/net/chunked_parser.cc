...
uint32_t ChunkedBodyParser::get_chunk_size(const string& chunk_hdr) const {
    ...
    if (space_loc != string::npos)
        hex_string.erase(space_loc, hex_string.length() - space_loc);
...
string::size_type ChunkedBodyParser::read(const std::string& input_buffer) {
    ...
    while (!parser_buffer_.empty()) {
        ...
        case CHUNK_HDR: {
            ...
            if (it != string::npos) {
                /* if you have CRLF, get chunk size & transition to
                 * CHUNK/TRAILER */
                current_chunk_size_ =
                    get_chunk_size(parser_buffer_.substr(0, it + 2));
...
/*
   ...
   successfully parsed.
*/
string::size_type ChunkedBodyParser::compute_ack_size(
