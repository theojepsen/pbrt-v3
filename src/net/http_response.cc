...
MIMEType::MIMEType(const string& content_type) : type_(), parameters_() {
...
string HTTPResponse::status_code() const {
    ...
    if (tokens.size() < 3) {
        throw runtime_error("HTTPResponse: Invalid status line: " +
                            first_line_);
    }
...
void HTTPResponse::calculate_expected_body_size() {
    ...
    if (status_code().at(0) == '1' or status_code() == "204" or
        status_code() == "304" or request_.is_head()) {
        /* Rule 1: size known to be zero */
        set_expected_body_size(true, 0);
    } else if (has_header("Transfer-Encoding") and
               equivalent_strings(
                   split(get_header_value("Transfer-Encoding"), ",").back(),
                   "chunked")) {
        /* Rule 2: size dictated by chunked encoding */
        /* Rule 2 is a bit ambiguous, but we think section 3.6 makes this
         * acceptable */

        set_expected_body_size(false);

        /* Create body_parser_ with trailers_enabled if requied (RFC 2616
         * section 14.40) */
        body_parser_ = make_unique<ChunkedBodyParser>(has_header("Trailer"));
    } else if ((not has_header("Transfer-Encoding")) and
               has_header("Content-Length")) {
        /* Rule 3: content-length header present to specify size */
        set_expected_body_size(true,
                               strict_atoi(get_header_value("Content-Length")));
    } else if (has_header("Content-Type") and
               equivalent_strings(
        ...
        set_expected_body_size(false);
        throw runtime_error(
            "HTTPResponse: unsupported multipart/byteranges without "
            "Content-Length");
    } else {
        /* Rule 5 */
        set_expected_body_size(false);
