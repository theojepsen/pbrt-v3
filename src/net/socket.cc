...
Address Socket::get_address(
    ...
    CheckSystemCall(name_of_function,
...
void Socket::connect(const Address &address) {
    CheckSystemCall(
        "connect", ::connect(fd_num(), &address.to_sockaddr(), address.size()));
    register_write();
}
...
void Socket::connect_nonblock(const Address &address) {
    try {
        connect(address);
        throw runtime_error(
            "nonblocking connect unexpectedly succeeded immediately");
...
void UDPSocket::sendto(const Address &destination, const string &payload) {
    const ssize_t bytes_sent = CheckSystemCall(
...
pair<Address, string> UDPSocket::recvfrom(void) {
    ...
    ssize_t recv_len = CheckSystemCall(
