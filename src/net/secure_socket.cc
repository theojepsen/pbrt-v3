...
SecureSocket::SecureSocket(TCPSocket &&sock, SSL *ssl)
    ...
    /* enable read/write to return only after handshake/renegotiation and
...
SecureSocket SSLContext::new_secure_socket(TCPSocket &&sock) {
    return SecureSocket(move(sock), SSL_new(ctx_.get()));
}
...
void SecureSocket::accept(const bool register_as_write) {
    ...
    if (ret == 1) {
        ...
        if (error_return == SSL_ERROR_WANT_WRITE or
            error_return == SSL_ERROR_WANT_READ) {
    ...
    if (bytes_read == 0) {
        if (SSL_ERROR_ZERO_RETURN == error_return) { /* Clean SSL close */
            set_eof();
                   ...
                   error_return) { /* Underlying TCP connection close */
...
void SecureSocket::write(const string &message, const bool register_as_read) {
    /* SSL_write returns with success if complete contents of message are
     * written */
    ERR_clear_error();
    ssize_t bytes_written =
        SSL_write(ssl_.get(), message.data(), message.length());
